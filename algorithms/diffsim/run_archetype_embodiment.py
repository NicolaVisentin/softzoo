...
def main():
    ...
    for it in range(args.n_iters):
        try:
            ...
            for frame in tqdm.tqdm(range(args.n_frames), desc=f'Forward #{it:04d}'):
                ...
                episode_time.append(obs["time"].clone())
                episode_objective_obs.append(obs["objective"].clone())
                ep_reward += reward
            ...
            episode_time = torch.stack(episode_time, dim=0).squeeze(1)
            episode_objective_obs = torch.stack(episode_objective_obs, dim=0)
    ...
    ax[0].plot(np.arange(len(data_for_plots['reward'])), data_for_plots['reward'])
    ax[1].set_title('Loss')
...
def make_parser():
    ...
    parser.add_argument('--objective', type=str, default='embody_archetype', choices=["move_forward", "embody_archetype"])
